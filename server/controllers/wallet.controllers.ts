...
const getBalance = async (req:any, res:any) => {
  const wallet:any = await Wallet.findOne({userId: req.user._id}).populate(
    'userId'
  );
...
const depositAmount = async (req:any, res:any) => {
  const {error}:any = ValidateDeposit(req.body);
...
const transferAmount = async (req:any, res:any, next:any) => {
  let session;

  try {
    const {error}:any = ValidateTransfer(req.body);
    if (error) throw new ApiError(400, error.issues[0].message, []);
    ...
    const senderWallet:any = await Wallet.findOne({userId: req.user._id}).session(
      session
    );
