...
const walletSchema = new mongoose.Schema({
  ...
  balance: {
    ...
    required: true,
    min: 0,
    set: (amount:number) => Math.round(amount * 100),
  },
  gainInBalance: {
    type: [Number],
    default: [100],
...
walletSchema.methods.depositGain = async function (amount:number) {
...
walletSchema.methods.transferGain = async function (amount:number) {
